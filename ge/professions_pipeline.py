...
def find_today_json() -> Path:
    ...
    candidates = [
        ...
        Path("data") / "ge" / padded,
        Path("ge") / padded,
        Path("scrappers") / "ge" / padded,
        Path("data") / padded,
        Path("scrappers") / padded,
        Path(unpadded),
        Path("data") / "ge" / unpadded,
        Path("ge") / unpadded,
        Path("scrappers") / "ge" / unpadded,
        Path("data") / unpadded,
    ...
    raise FileNotFoundError(
        f"Nao encontrei ficheiro do dia: {padded} (ou {unpadded}) na raiz/ge/data/scrappers."
    )
...
def find_default_input_and_output(input_arg: str, output_arg: str) -> tuple[Path, Path]:
    ...
    fixed_candidates = [
        ...
        Path("data") / "ge" / "professions.json",
        Path("ge") / "professions.json",
        Path("scrappers") / "ge" / "professions.json",
    ]
    ...
    default_out = Path("data") / "ge" / "professions.json"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "data/ge/professions.json (ou cria a partir do ficheiro do dia)."
        ),
    )
    parser.add_argument(
        "--output",
