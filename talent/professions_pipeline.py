...
2) usa title + description para classificar profissoes (OpenAI)
...
def find_today_json() -> Path:
    ...
    candidates = [
        Path(padded),
        Path("data") / "talent" / padded,
        Path("scrappers") / "talent" / padded,
        Path("data") / padded,
        Path("scrappers") / padded,
        Path(unpadded),
        Path("data") / "talent" / unpadded,
        Path("scrappers") / "talent" / unpadded,
        Path("data") / unpadded,
    ...
    raise FileNotFoundError(
        f"Nao encontrei ficheiro do dia: {padded} (ou {unpadded}) na raiz/data/scrappers."
    )
...
def find_default_input_and_output(input_arg: str, output_arg: str) -> tuple[Path, Path]:
    ...
    fixed_candidates = [
        ...
        Path("data") / "talent" / "professions.json",
        Path("scrappers") / "talent" / "professions.json",
    ]
    ...
    default_out = Path("data") / "talent" / "professions.json"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "data/talent/professions.json (ou cria a partir do ficheiro do dia)."
        ),
    )
    parser.add_argument(
        "--output",
        default="",
        help="JSON de saida. Se omitido, atualiza no proprio ficheiro de trabalho.",
    )
    parser.add_argument(
        "--model",
        default="gpt-4.1",
        help="Modelo OpenAI para classificacao.",
    )
    parser.add_argument(
        "--log-file",
        default="",
    ...
    parser.add_argument(
        "--force-all",
        action="store_true",
        help="Reprocessa todos os registos, mesmo os que ja tem professions.",
    )
    parser.add_argument(
        "--retry-errors",
        action="store_true",
        help="Tambem reprocessa registos que tem professions_error.",
    )
