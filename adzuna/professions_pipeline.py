...
def find_today_json() -> Path:
    ...
    candidates = [
        Path(padded),
        Path("data") / "adzuna" / padded,
        Path("adzuna") / padded,
        Path(unpadded),
        Path("data") / "adzuna" / unpadded,
        Path("adzuna") / unpadded,
    ]
    for c in candidates:
        if c.exists():
            return c
    raise FileNotFoundError(
        f"Nao encontrei ficheiro do dia: {padded} (ou {unpadded}) na raiz/data/adzuna."
    )
...
def find_default_input_and_output(input_arg: str, output_arg: str) -> tuple[Path, Path]:
    ...
    fixed_candidates = [
        ...
        Path("data") / "adzuna" / "professions.json",
        Path("adzuna") / "professions.json",
    ]
    ...
    default_out = Path("data") / "adzuna" / "professions.json"
...
def main() -> None:
    ...
    parser.add_argument(
        "--input",
        default="",
        help="JSON de entrada. Se omitido, usa data/adzuna/professions.json.",
    )
    parser.add_argument(
        "--output",
        default="",
